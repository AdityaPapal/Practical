...
void solveNQueensBranchAndBound(int row, vector<int>& board, int& count) {
    int n = board.size();
    if (row == n) {
        count++;
        cout << "Solution " << count << ":\n";
    ...
    for (int col = 0; col < n; ++col) {
        if (promising(row, col, board)) {
            board[row] = col;
            solveNQueensBranchAndBound(row + 1, board, count);
            board[row] = -1;
        }
    }
}
...
int main() {
    ...
    count = 0; // Reset count for branch and bound

    cout << "\nBranch and Bound Solutions:\n";
    solveNQueensBranchAndBound(0, board, count);
    if (count == 0) {
        cout << "No solutions found using branch and bound.\n";
    }
